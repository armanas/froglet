...
: "${FROGLET_GCP_DISCOVERY_MACHINE_TYPE:=e2-standard-2}"
...
role_machine_type() {
  case "$1" in
    ...
    froglet-discovery) echo "$FROGLET_GCP_DISCOVERY_MACHINE_TYPE" ;;
...
role_has_public_edge() {
  case "$1" in
    froglet-marketplace|froglet-provider-free|froglet-provider-paid|froglet-discovery) return 0 ;;
    *) return 1 ;;
  esac
}
  ...
  cat >"$(role_meta_path "$role")" <<EOF
{
  ...
  "operator_url": "http://127.0.0.1:9191",
  "token_paths": {
...
for meta_path in sorted((state_dir / "meta").glob("*.json")):
    ...
    if role["role"] != "froglet-settlement-lab":
        role["provider_public_url"] = None if role["role"] == "froglet-discovery" else f'https://{role["nat_ip"]}'
...
inventory = {
    ...
    "discovery_url": f'https://{roles["froglet-discovery"]["nat_ip"]}',
    "roles": roles,
}
...
render_stack_env() {
  local role="$1"
  local nat_ip discovery_url data_root
  nat_ip="$(python3 - "$(role_meta_path "$role")" <<'PY'
import json, sys
print(json.load(open(sys.argv[1], "r", encoding="utf-8"))["nat_ip"])
PY
)"
  discovery_url="$(inventory_get discovery_url)"
  data_root="$(role_data_root "$role")"
  cat <<EOF
FROGLET_DATA_ROOT=$data_root
FROGLET_LISTEN_ADDR=127.0.0.1:8080
FROGLET_RUNTIME_LISTEN_ADDR=127.0.0.1:8081
FROGLET_PROVIDER_CONTROL_LISTEN_ADDR=127.0.0.1:9191
FROGLET_OPERATOR_PROVIDER_BASE_URL=http://127.0.0.1:8080
FROGLET_OPERATOR_RUNTIME_BASE_URL=http://127.0.0.1:8081
FROGLET_RUNTIME_PROVIDER_BASE_URL=http://127.0.0.1:8080
FROGLET_OPERATOR_PROVIDER_LOG_PATH=$REMOTE_ROOT/logs/provider.log
FROGLET_OPERATOR_RUNTIME_LOG_PATH=$REMOTE_ROOT/logs/runtime.log
FROGLET_OPERATOR_PROVIDER_RESTART_COMMAND=sudo -n /usr/bin/systemctl restart froglet-provider.service
FROGLET_OPERATOR_RUNTIME_RESTART_COMMAND=sudo -n /usr/bin/systemctl restart froglet-runtime.service
FROGLET_PUBLIC_BASE_URL=https://$nat_ip
FROGLET_HTTP_CA_CERT_PATH=$REMOTE_ROOT/pki/ca.pem
FROGLET_DISCOVERY_MODE=reference
FROGLET_DISCOVERY_URL=$discovery_url
FROGLET_DISCOVERY_PUBLISH=true
FROGLET_DISCOVERY_REQUIRED=true
EOF
  case "$role" in
    froglet-provider-paid)
      cat <<EOF
...
render_discovery_env() {
  cat <<EOF
FROGLET_DISCOVERY_LISTEN_ADDR=127.0.0.1:9090
EOF
}
...
operator_unit() {
  cat <<EOF
[Unit]
Description=Froglet Operator
After=network-online.target froglet-provider.service froglet-runtime.service
Wants=network-online.target
...
User=$FROGLET_GCP_REMOTE_USER
WorkingDirectory=$REMOTE_ROOT/repo
EnvironmentFile=$REMOTE_ROOT/etc/discovery.env
ExecStart=/bin/bash -lc 'exec "$REMOTE_ROOT/bin/froglet-discovery" >>"$REMOTE_ROOT/logs/discovery.log" 2>&1'
Restart=always
RestartSec=2
...
sudoers_snippet() {
  cat <<EOF
$FROGLET_GCP_REMOTE_USER ALL=(root) NOPASSWD: /usr/bin/systemctl restart froglet-provider.service, /usr/bin/systemctl restart froglet-runtime.service
EOF
}
...
configure_role() {
  ...
  if role_has_stack "$role"; then
    ...
    install_remote_unit "$role" froglet-operator.service "$(operator_unit)"
    local sudoers_file="$STATE_DIR/tmp/$role-sudoers"
    sudoers_snippet >"$sudoers_file"
    install_remote_text_file "$role" "$sudoers_file" "/etc/sudoers.d/froglet-harness" 0440
  fi

  if [[ "$role" == "froglet-discovery" ]]; then
    local discovery_env="$STATE_DIR/tmp/$role-discovery.env"
    render_discovery_env >"$discovery_env"
    gcp_scp_to "$role" "$discovery_env" "$REMOTE_ROOT/etc/discovery.env"
    install_remote_unit "$role" froglet-discovery.service "$(discovery_unit)"
  fi
...
start_role_services() {
  local role="$1"
  if [[ "$role" == "froglet-discovery" ]]; then
    gcp_ssh "$role" "sudo systemctl enable --now froglet-discovery.service froglet-public-edge.service"
    return
  fi
  if role_has_stack "$role"; then
    gcp_ssh "$role" "sudo systemctl enable --now froglet-provider.service froglet-runtime.service froglet-operator.service"
  fi
  if role_has_public_edge "$role"; then
    gcp_ssh "$role" "sudo systemctl enable --now froglet-public-edge.service"
  fi
}
...
wait_role_health() {
  ...
  if [[ "$role" == "froglet-discovery" ]]; then
    while [[ $SECONDS -lt $deadline ]]; do
      if gcp_ssh "$role" "curl -fsS http://127.0.0.1:9090/health >/dev/null"; then
        return 0
      fi
      sleep 2
    done
  elif role_has_stack "$role"; then
    while [[ $SECONDS -lt $deadline ]]; do
      if gcp_ssh "$role" "curl -fsS http://127.0.0.1:8080/health >/dev/null && curl -fsS http://127.0.0.1:8081/health >/dev/null && curl -fsS http://127.0.0.1:9191/health >/dev/null"; then
        return 0
      fi
      sleep 2
    done
...
copy_remote_tokens_locally() {
  ...
  chmod 0600 "$TOKENS_DIR/provider.token" "$TOKENS_DIR/consumer.token" "$TOKENS_DIR/bogus.token"
}
...
wait_for_reference_discovery_convergence() {
...
PY
  ...
  gcp_ssh "froglet-discovery" "curl -fsS http://127.0.0.1:9090/health >/dev/null"
...
while time.time() < deadline:
    try:
        status = request_json('http://127.0.0.1:9191/v1/froglet/status')
        if status.get('reference_discovery', {}).get('connected') is not True:
            last_error = f\"reference discovery not connected: {status.get('reference_discovery')}\"
            time.sleep(2)
            continue
        discovered = request_json('http://127.0.0.1:9191/v1/froglet/services/discover', {'limit': 20})
        provider_ids = {entry.get('provider_id') for entry in discovered.get('services', []) if entry.get('provider_id')}
        if os.environ['FREE_PROVIDER_ID'] in provider_ids and os.environ['PAID_PROVIDER_ID'] in provider_ids:
            print('ready')
            break
        last_error = f'provider discovery incomplete: {sorted(provider_ids)}'
    except (OSError, urllib.error.URLError, urllib.error.HTTPError, json.JSONDecodeError) as error:
        last_error = str(error)
    time.sleep(2)
else:
    raise SystemExit(f'timed out waiting for discovery convergence: {last_error}')
PY
  "
}
...
cmd_deploy() {
  ...
  start_role_services "froglet-discovery"
  wait_role_health "froglet-discovery"
...
cmd_run_matrix() {
  ...
  node tests/e2e/gcp_harness/run-matrix.mjs \
    --inventory "$INVENTORY_PATH" \
  ...
  "$STATE_DIR/.venv/bin/python" tests/e2e/gcp_harness/protocol_matrix.py \
    ...
    --seed-free "$SEED_FREE_PATH" \
    --seed-paid "$SEED_PAID_PATH" \
    --operator-url "http://127.0.0.1:19191" \
  ...
  trap - RETURN
  close_marketplace_tunnel
  if [[ "$tool_status" -ne 0 || "$protocol_status" -ne 0 || "$lnd_status" -ne 0 ]]; then
    return 1
  fi
}
...
cmd_run_agentic() {
  ...
  generate_scenario_set "$(generate_execution_suffix)"
  local api_key
  api_key="$(gcloud secrets versions access latest --project="$FROGLET_GCP_PROJECT" --secret="$FROGLET_GCP_OPENAI_SECRET")"
  gcp_ssh "froglet-marketplace" "
    set -euo pipefail
    cd '$REMOTE_ROOT/repo'
    ...
    node tests/e2e/gcp_harness/run-agentic.mjs \
      ...
      --auth-token-path '$REMOTE_ROOT/data/froglet-marketplace/runtime/froglet-control.token' \
...
if agentic:
    ...
    lines.append(f"- Agentic deterministic scenarios: {len(agentic['deterministic'])}")

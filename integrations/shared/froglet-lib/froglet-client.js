...
const HASH_VERIFY_EXAMPLE_SHA256 = "fb91d75a6bb430787a61b0aec5e374f580030f2878e1613eab5ca6310f7bbb9a"
...
const HOSTED_PROOF_FOLLOWUPS = new Set(["none", "demo.fetch-witness", "demo.hash-verify"])
...
async function readAuthToken(tokenPath) {
  if (typeof tokenPath !== "string" || tokenPath.trim().length === 0) {
    throw new Error(
      "froglet auth token path is not configured; set FROGLET_PROVIDER_AUTH_TOKEN_PATH/FROGLET_RUNTIME_AUTH_TOKEN_PATH for local actions, or use run_hosted_proof for the zero-config hosted demo"
    )
  }
...
function hostedServiceById(services, serviceId) {
  const service = (Array.isArray(services) ? services : []).find(
    (entry) => entry?.service_id === serviceId || entry?.offer_id === serviceId
  )
  if (!service) {
    throw new Error(`hosted proof service not found: ${serviceId}`)
  }
  return service
}

function hostedProviderForService(service) {
  const providerId = nonEmptyString(service?.provider_id)
  if (!providerId) {
    throw new Error(`hosted proof service ${service?.service_id ?? "unknown"} did not include provider_id`)
  }
  return {
    provider_id: providerId,
    provider_url: nonEmptyString(service?.provider_url) ?? "https://ai.froglet.dev",
  }
}

function hostedDealRequestBody(service, input) {
  return {
    provider: hostedProviderForService(service),
    offer_id: service?.offer_id ?? service?.service_id,
    kind: "execution",
    execution: buildServiceAddressedExecution(service, input),
  }
}

function hostedDealId(payload) {
  const dealId = payload?.deal?.deal_id ?? payload?.deal_id
  if (typeof dealId !== "string" || dealId.trim().length === 0) {
    throw new Error(`hosted proof deal create response did not include deal.deal_id: ${JSON.stringify(payload)}`)
  }
  return dealId
}

function hostedDealStatus(payload) {
  const status = payload?.deal?.status ?? payload?.status
  return typeof status === "string" ? status : "unknown"
}

function hostedDealResult(payload) {
  return payload?.deal?.result ?? payload?.result ?? null
}

function hostedDealHasReceipt(payload) {
  return Boolean(payload?.deal?.receipt ?? payload?.receipt)
}

async function createHostedDeal({ hostedUrl, token, requestTimeoutMs, service, input }) {
  const body = hostedDealRequestBody(service, input)
  const response = await jsonRequest(`${hostedUrl}/v1/runtime/deals`, {
    method: "POST",
...
async function pollHostedDeal({ hostedUrl, token, requestTimeoutMs, dealId, maxAttempts = 8 }) {
  let response = null
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    response = await jsonRequest(`${hostedUrl}/v1/runtime/deals/${encodeURIComponent(dealId)}`, {
      timeoutMs: requestTimeoutMs,
      headers: hostedAuthHeaders(token),
    })
    if (TERMINAL_DEAL_STATES.has(hostedDealStatus(response.payload).toLowerCase())) {
      return response
    }
    await new Promise((resolve) => setTimeout(resolve, Math.min(250 * (attempt + 1), 1000)))
  }
  throw new Error(
    `hosted proof deal ${dealId} did not reach a terminal status after ${maxAttempts} polls; last status: ${hostedDealStatus(response?.payload)}`
  )
}

function hostedFollowupInput(serviceId) {
  if (serviceId === "demo.fetch-witness") {
    return { url: "https://example.com/", max_bytes: 1048576 }
  }
  if (serviceId === "demo.hash-verify") {
    return {
      url: "https://example.com/",
      expected_sha256: HASH_VERIFY_EXAMPLE_SHA256,
    }
  }
  throw new Error(`unsupported hosted proof follow-up: ${serviceId}`)
}

async function runHostedServiceProof({ hostedUrl, token, requestTimeoutMs, service, input }) {
  const create = await createHostedDeal({ hostedUrl, token, requestTimeoutMs, service, input })
  const dealId = hostedDealId(create.payload)
  const poll = await pollHostedDeal({ hostedUrl, token, requestTimeoutMs, dealId })
  return {
    service_id: service?.service_id ?? service?.offer_id ?? "unknown",
    create_status: create.status,
    poll_status: poll.status,
    deal_id: dealId,
    deal_status: hostedDealStatus(poll.payload),
    result: hostedDealResult(poll.payload),
    receipt_present: hostedDealHasReceipt(poll.payload),
    request_body: create.body,
    poll_payload: poll.payload,
  }
}

async function tryHostedFollowupProof({ hostedUrl, token, requestTimeoutMs, service, input }) {
  try {
    return await runHostedServiceProof({
      hostedUrl,
      token,
      requestTimeoutMs: Math.min(requestTimeoutMs, 10000),
      service,
      input,
    })
  } catch (error) {
...
function hostedFeedSummary(response) {
  const payload = response.payload
  const artifacts = Array.isArray(payload?.artifacts) ? payload.artifacts : []
  return {
    status: response.status,
    artifact_envelope: Array.isArray(payload?.artifacts),
    artifact_count: artifacts.length,
    artifact_kinds: [...new Set(artifacts.map((artifact) => artifact?.kind).filter(Boolean))],
    has_events: Array.isArray(payload?.events),
...
async function optionalHostedPreflight(hostedUrl, requestTimeoutMs) {
  try {
    const response = await jsonRequest(`${hostedUrl}/api/preflight`, {
      timeoutMs: Math.min(requestTimeoutMs, 3000),
    })
    return {
      status: response.status,
...
export async function runHostedProof({
  hostedUrl = HOSTED_TRIAL_URL,
  requestTimeoutMs = 10000,
  followup = "demo.fetch-witness",
} = {}) {
  const normalizedHostedUrl = normalizeUrl(hostedUrl) ?? HOSTED_TRIAL_URL
  if (!HOSTED_PROOF_FOLLOWUPS.has(followup)) {
    throw new Error(`followup must be one of: ${[...HOSTED_PROOF_FOLLOWUPS].join(", ")}`)
  }

  const preflight = await optionalHostedPreflight(normalizedHostedUrl, requestTimeoutMs)
  const session = await jsonRequest(`${normalizedHostedUrl}/api/sessions`, {
    method: "POST",
    timeoutMs: requestTimeoutMs,
  })
  const token = session.payload?.session_token
  if (typeof token !== "string" || token.length === 0) {
    throw new Error("hosted proof session response did not include session_token")
  }
  const servicesResponse = await jsonRequest(`${normalizedHostedUrl}/v1/provider/services`, {
    timeoutMs: requestTimeoutMs,
    headers: hostedAuthHeaders(token),
  })
  const services = Array.isArray(servicesResponse.payload?.services)
    ? servicesResponse.payload.services
    : []
  const addService = hostedServiceById(services, "demo.add")
  const add = await runHostedServiceProof({
  ...
  const followupProof =
    followup === "none"
      ? null
      : await tryHostedFollowupProof({
          hostedUrl: normalizedHostedUrl,
          token,
          requestTimeoutMs,
          service: hostedServiceById(services, followup),
          input: hostedFollowupInput(followup),
        })

  const feed = await jsonRequest(`${normalizedHostedUrl}/v1/feed`, {
    timeoutMs: requestTimeoutMs,
    headers: hostedAuthHeaders(token),
  })

  return {
    hosted_url: normalizedHostedUrl,
    preflight,
    session: {
      status: session.status,
      slot_id: session.payload?.slot_id ?? null,
      ttl_secs: session.payload?.ttl_secs ?? null,
    },
    services: {
      status: servicesResponse.status,
      service_ids: services.map((service) => service?.service_id ?? service?.offer_id).filter(Boolean),
      demo_services: services
        .map((service) => service?.service_id ?? service?.offer_id)
        .filter((serviceId) => typeof serviceId === "string" && serviceId.startsWith("demo.")),
    },
    demo_add: add,
    followup: followupProof,
    feed: hostedFeedSummary(feed),
  }
}

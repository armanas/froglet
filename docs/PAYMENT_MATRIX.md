...
| --- | --- | --- | --- |
| `None` | [none.rs](../src/settlement/none.rs) | — | Free-only deals. Used in local compose smoke, conformance tests, and the public trial. |
| `Lightning` | [lightning.rs](../src/settlement/lightning.rs) | `Mock`, `LndRest` | BOLT11 invoices. `Mock` is deterministic + in-memory for unit tests; `LndRest` talks to any LND REST endpoint (regtest, signet, or mainnet). |
| `X402` | [x402.rs](../src/settlement/x402.rs) | — | HTTP 402 challenge/response; a lightweight cryptographic settlement rail suitable for agent-to-agent calls. |
| `Stripe` | [stripe.rs](../src/settlement/stripe.rs) | — | Fiat via Stripe PaymentIntents (Multi-Party Payments / Stripe Connect). |
...
# Hosted cells run once `ai.froglet.dev` is serving the Froglet provider
# publicly.
```

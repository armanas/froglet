...
That writes `~/.froglet/agent/compose.yaml` using published GHCR images for
the latest release and starts provider/runtime without cloning the repo. MCP
config is generated by the same bootstrap and backed by the published
`froglet-mcp` image.
